...
using FactDecisionValue = Aevatar.VibeResearching.Agents.Contracts.Collab.FactDecisionValue;
using FactVoteValue = Aevatar.VibeResearching.Agents.Contracts.Collab.FactVoteValue;
...
public sealed class FactLifecycleService : IFactLifecycleService
{
    ...
    private readonly IDagRepository _dag;
    private readonly ILogger<FactLifecycleService> _logger;

    public FactLifecycleService(IWorkspaceService workspace, IDagRepository dag, ILogger<FactLifecycleService> logger)
    {
        ...
        CancellationToken ct)
    {
        ...
        if (decision == Aevatar.VibeResearching.Knowledge.FactDecisionValue.Unspecified)
        ...
        var final = new FactDecision
        {
            SessionId = ws.SessionId,
            FactId = factId,
            Decision = (global::Aevatar.VibeResearching.Agents.Contracts.Collab.FactDecisionValue)(int)decision,
            Basis = basis,
            Rationale = rationale,
            FinalizedAt = now,
        ...
        if (decision == Aevatar.VibeResearching.Knowledge.FactDecisionValue.Promote)
    ...
    private static (Aevatar.VibeResearching.Knowledge.FactDecisionValue decision, string basis, string rationale) Decide(
        IReadOnlyList<FactVote> votes,
        IReadOnlyList<FactVerification> verifs)
    {
        if (verifs.Any(v => v.Result))
        {
            return (Aevatar.VibeResearching.Knowledge.FactDecisionValue.Promote, "verification", "hard verification passed");
        }

        var approvals = votes.Count(v => v.Vote == global::Aevatar.VibeResearching.Agents.Contracts.Collab.FactVoteValue.Approve);
        var rejects = votes.Count(v => v.Vote == global::Aevatar.VibeResearching.Agents.Contracts.Collab.FactVoteValue.Reject);

        if (approvals >= RequiredApprovals)
            return (Aevatar.VibeResearching.Knowledge.FactDecisionValue.Promote, "votes", $"approvals={approvals} (threshold={RequiredApprovals})");

        if (rejects >= RequiredRejects)
            return (Aevatar.VibeResearching.Knowledge.FactDecisionValue.Reject, "votes", $"rejects={rejects} (threshold={RequiredRejects})");

        return (Aevatar.VibeResearching.Knowledge.FactDecisionValue.Unspecified, "", "");
    }
