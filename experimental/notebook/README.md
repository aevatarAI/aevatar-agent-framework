...
编辑 `experimental/notebook/src/Aevatar.Notebook.Api/appsettings.json`：
...
数据库连接信息建议放到用户级 secrets（加密，推荐：`~/.aevatar/secrets.json`；用 `src/Aevatar.Agents.SecretsCli` 写入）或 `experimental/notebook/src/Aevatar.Notebook.Api/appsettings.secrets.json`（已被 `.gitignore` 忽略）。
可直接复制示例：

- `experimental/notebook/src/Aevatar.Notebook.Api/appsettings.secrets.json.example`
...
把下面内容放到用户级 secrets（推荐）或 `experimental/notebook/src/Aevatar.Notebook.Api/appsettings.secrets.json`（或环境变量注入同名配置）：
...
（可选）如果你想减少网络层日志噪音：`experimental/notebook/src/Aevatar.Notebook.Api/appsettings.json` 已把 `System.ClientModel*` 调到 `Warning`。
