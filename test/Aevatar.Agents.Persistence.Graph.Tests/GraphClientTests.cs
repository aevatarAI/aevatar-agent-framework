...
public class GraphClientTests
{
    ...
    public async Task WriteNode_returns_backend_id()
    {
        ...
        var client = new GraphClient(_compiler, _executor);
    ...
    private sealed class FakeCompiler : IGraphCompiler<CypherCommand>
    {
        public CypherCommand Compile(GraphPlan plan) =>
            new("noop", new Dictionary<string, object?>(), plan.Operation);
    }

    private sealed class FakeExecutor : IGraphExecutor<CypherCommand>
    {
        public Func<GraphOperation, object?>? OnExecute { get; set; }

        public Task<object?> ExecuteAsync(CypherCommand compiled)
        {
