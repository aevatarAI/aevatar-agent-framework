...
- Agent Skills（`SKILL.md`）负责 SOP/模板/rubric 按需加载（见 `experimental/novel/agent_skills/`）
...
原则：**API 的数据结构必须对应 Protobuf 契约**（见 `experimental/novel/protos/*.proto`），即使前端先用 JSON 映射，也不能出现手写 DTO 漂移。
...
当能力本质是 SOP/模板/评分 rubric，需要按需加载并可独立迭代时，用 Skill（`experimental/novel/agent_skills/*/SKILL.md`）：
...
- Protobuf 契约目录：`experimental/novel/protos/`
  - 资产与配置：`experimental/novel/protos/novel_assets.proto`
  - 流水线/事件：`experimental/novel/protos/novel_pipeline.proto`
- Skills 目录：`experimental/novel/agent_skills/`
