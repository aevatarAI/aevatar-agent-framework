...
namespace ScientificResearchAssistant.Api.Facts;

// ============================================================
//  FactLifecycleService (facts_proposed -> decisions -> facts)
//
//  MVP policy:
...
//  - Facts are stored in system-level `facts/` (shared knowledge base).
...
public sealed class FactLifecycleService
{
    ...
    public FactLifecycleService(WorkspaceService workspace, ILogger<FactLifecycleService> logger)
    {
        ...
        CancellationToken ct)
    {
        ...
        {
            await PromoteToFactsAsync(ws, factId, ct);
        }
    ...
    private async Task PromoteToFactsAsync(WorkspacePaths ws, string factId, CancellationToken ct)
    {
        ct.ThrowIfCancellationRequested();

        Directory.CreateDirectory(ws.FactsDir);

        var src = Path.Combine(ws.FactsProposedDir, $"{factId}.json");
        var dst = Path.Combine(ws.FactsDir, $"{factId}.json");
        ...
        // Copy is enough for MVP; keep proposed record for audit.
