...
using ProtoPaperTargetFile = Aevatar.VibeResearching.Agents.Contracts.Collab.PaperTargetFile;
using ProtoPaperPatchFormat = Aevatar.VibeResearching.Agents.Contracts.Collab.PaperPatchFormat;
using DomainPaperTargetFile = Aevatar.VibeResearching.Infrastructure.PaperTargetFile;
using DomainPaperPatchFormat = Aevatar.VibeResearching.Infrastructure.PaperPatchFormat;

namespace Aevatar.VibeResearching.Infrastructure.MongoDB.Paper;
...
public sealed class PaperService : Aevatar.VibeResearching.Infrastructure.IPaperService
{
    ...
    public async Task ApplyPatchAsync(string sessionId, string? runId, PaperPatchProposal proposal, CancellationToken ct)
    {
        ...
        var targetFile = ResolveTargetPath(ws, (DomainPaperTargetFile)proposal.TargetFile);

        // Deterministic, bounded patch.
        switch ((DomainPaperPatchFormat)proposal.Format)
        {
            case DomainPaperPatchFormat.ReplaceSpan:
    ...
    private static string ResolveTargetPath(WorkspacePaths ws, DomainPaperTargetFile target)
    {
        return target switch
        {
            DomainPaperTargetFile.Outline => ws.PaperOutlinePath,
            DomainPaperTargetFile.Draft => ws.PaperDraftPath,
