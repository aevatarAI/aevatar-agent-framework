...
public class AevatarConfigLoader
{
    ...
    public AevatarConfig Load()
    {
        ...
        // 3. 加载 Agent 配置
        config.Agents = LoadAgentConfigs();
...
public class WorkflowEngine
{
        ...
        IReadOnlyList<NodeSpec> nodes)
    {
        ...
        {
            var agentType = ResolveAgentType(node.Type);
            var actor = await _actorFactory.CreateGAgentActorAsync(
                agentType, 
                Guid.NewGuid()
            );
            ...
            if (agent is IConfigurableAgent configurable)
            {
                configurable.Configure(node.Params);
            }
...
```
opencode/                              # 重命名为 aevatar-platform?
├── docs/
│   ├── PRD.md
│   └── FEASIBILITY.md
│
