...
using Aevatar.Agents.Cognitive.Content;
using Aevatar.Agents.Cognitive.Core;
using Aevatar.Agents.Cognitive.Core.Tasks;
...
builder.Services.AddMassTransitStreamPlugin(
    ...
    typeof(Aevatar.Agents.Cognitive.Agents.CoordinatorAgent).Assembly      // DSL 驱动的认知系统
);
...
{
    var templates = Enum.GetValues<Aevatar.Agents.Cognitive.Core.Tasks.TaskTemplate>()
        .Select(t => new
        {
            value = t.ToString(),
