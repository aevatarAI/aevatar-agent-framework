...
{
  "name": "weex_get_order",
  "description": "Get order details from WEEX (authenticated). Reads credentials from env vars: WEEX_API_KEY, WEEX_API_SECRET, WEEX_PASSPHRASE. Optional: WEEX_BASE_URL (default https://api-spot.weex.com).",
  "category": "Custom",
  "version": "0.1.0",
  "tags": ["weex", "trade", "order", "query"],
...
if (!string.IsNullOrWhiteSpace(orderId)) query += $"&orderId={Uri.EscapeDataString(orderId)}";
if (!string.IsNullOrWhiteSpace(clientOrderId)) query += $"&clientOid={Uri.EscapeDataString(clientOrderId)}";

var path = $"/api/v2/trade/order?{query}";
