...
// ============================================================
//  Aevatar.Config / aevatar-config
//
//  A local web UI to configure Aevatar user secrets.
//
//  Tool usage:
//    aevatar-config [--no-browser] [--port <port>]
//
//  Default URLs:
//   - App:  http://localhost:6667 (unsafe in Chrome) + http://localhost:6677
//   - Tool: http://localhost:6677 (browser-friendly)
//
//  User secrets path:
...
// Configure port (tool single-port)
var url = $"http://localhost:{port}";
builder.WebHost.UseUrls(url);
#else
var builder = WebApplication.CreateBuilder(args);

// App: if no URLs configured (ASPNETCORE_URLS / --urls), listen on:
// - 6667: requested default (Chrome blocks as unsafe port)
...
// ------------------------------------------------------------
// WebSearch providers (config via user secrets)
// Stored at: Aevatar:Tools:WebSearch:*  (fallback: WebSearch:*)
// ------------------------------------------------------------
app.MapGet("/api/websearch", (IAevatarUserSecretsStore secrets, HttpContext http) =>
{
    if (!IsLocal(http))
        return Results.Forbid();

    var status = ReadWebSearchStatus(secrets);
    ...
    return Results.Json(new
    {
        ...
        webSearch = new
        {
            enabled = status.EnabledSetting,
            effectiveEnabled = status.Enabled,
            provider = status.Provider,
            endpoint = status.Endpoint,
            timeoutMs = status.TimeoutMs,
            searchDepth = status.SearchDepth,
...
app.MapGet("/api/websearch/status", (IAevatarUserSecretsStore secrets, HttpContext http) =>
{
    if (!IsLocal(http))
        return Results.Forbid();
...
app.MapGet("/api/websearch/api-key", (
    bool? reveal,
    IAevatarUserSecretsStore secrets,
    HttpContext http) =>
{
    ...
    var raw = ReadWebSearchValue(secrets, "apiKey");
...
app.MapPost("/api/websearch", (
    UpsertWebSearchRequest req,
    IAevatarUserSecretsStore secrets,
    HttpContext http) =>
{
    if (!IsLocal(http))
    ...
    if (req.Enabled.HasValue)
    {
        SetWebSearchValue(secrets, "enabled", req.Enabled.Value ? "true" : "false");
    }

    SetWebSearchValue(secrets, "provider", req.Provider);
    SetWebSearchValue(secrets, "endpoint", req.Endpoint);
    SetWebSearchValue(secrets, "searchDepth", req.SearchDepth);

    if (req.TimeoutMs.HasValue)
    {
        if (req.TimeoutMs.Value <= 0)
            RemoveWebSearchKey(secrets, "timeoutMs");
        else
            SetWebSearchValue(secrets, "timeoutMs", req.TimeoutMs.Value.ToString());
    }

    // ApiKey: only write when provided (UI should prevent saving masked values).
    if (req.ApiKey != null)
    {
        SetWebSearchValue(secrets, "apiKey", req.ApiKey);
    }

    return Results.Json(new { ok = true });
});

app.MapDelete("/api/websearch", (
    IAevatarUserSecretsStore secrets,
    HttpContext http) =>
{
    if (!IsLocal(http))
    ...
    var removed = new Dictionary<string, bool>
    {
        ["Aevatar:Tools:WebSearch:enabled"] = RemoveWebSearchKey(secrets, "enabled"),
        ["Aevatar:Tools:WebSearch:provider"] = RemoveWebSearchKey(secrets, "provider"),
        ["Aevatar:Tools:WebSearch:endpoint"] = RemoveWebSearchKey(secrets, "endpoint"),
        ["Aevatar:Tools:WebSearch:timeoutMs"] = RemoveWebSearchKey(secrets, "timeoutMs"),
        ["Aevatar:Tools:WebSearch:searchDepth"] = RemoveWebSearchKey(secrets, "searchDepth"),
        ["Aevatar:Tools:WebSearch:apiKey"] = RemoveWebSearchKey(secrets, "apiKey")
    };
...
// ------------------------------------------------------------
// WebSearch helpers (Aevatar:Tools:WebSearch + WebSearch fallback)
// ------------------------------------------------------------
static string BuildWebSearchNamespacedKey(string key) => $"Aevatar:Tools:WebSearch:{key}";
static string BuildWebSearchRootKey(string key) => $"WebSearch:{key}";

static string? ReadWebSearchValue(IAevatarUserSecretsStore secrets, string key)
{
    var nsKey = BuildWebSearchNamespacedKey(key);
    if (secrets.TryGet(nsKey, out var nsVal) && !string.IsNullOrWhiteSpace(nsVal))
        return nsVal.Trim();

    var rootKey = BuildWebSearchRootKey(key);
    if (secrets.TryGet(rootKey, out var rootVal) && !string.IsNullOrWhiteSpace(rootVal))
        return rootVal.Trim();

    return null;
}

static bool TryReadWebSearchBool(IAevatarUserSecretsStore secrets, string key, out bool value)
{
    value = false;
    var raw = ReadWebSearchValue(secrets, key);
    if (string.IsNullOrWhiteSpace(raw))
        return false;
...
static int? ReadWebSearchInt(IAevatarUserSecretsStore secrets, string key)
{
    var raw = ReadWebSearchValue(secrets, key);
    if (string.IsNullOrWhiteSpace(raw))
        return null;
...
static void SetWebSearchValue(IAevatarUserSecretsStore secrets, string key, string? raw)
{
    if (raw == null)
        return;
...
static bool RemoveWebSearchKey(IAevatarUserSecretsStore secrets, string key)
{
    var nsKey = BuildWebSearchNamespacedKey(key);
    var rootKey = BuildWebSearchRootKey(key);
    var removed = secrets.Remove(nsKey);
    var removedRoot = secrets.Remove(rootKey);
    return removed || removedRoot;
}

static WebSearchStatus ReadWebSearchStatus(IAevatarUserSecretsStore secrets)
{
    var hasExplicitEnabled = TryReadWebSearchBool(secrets, "enabled", out var enabledValue);
    var enabledSetting = hasExplicitEnabled ? (bool?)enabledValue : null;
    var effectiveEnabled = hasExplicitEnabled ? enabledValue : true;

    var provider = (ReadWebSearchValue(secrets, "provider") ?? "tavily").Trim();
    if (string.IsNullOrWhiteSpace(provider))
        provider = "tavily";

    var endpoint = ReadWebSearchValue(secrets, "endpoint") ?? string.Empty;
    var searchDepth = ReadWebSearchValue(secrets, "searchDepth") ?? string.Empty;
    var timeoutMs = ReadWebSearchInt(secrets, "timeoutMs");

    var apiKeyRaw = ReadWebSearchValue(secrets, "apiKey");
    var configured = !string.IsNullOrWhiteSpace(apiKeyRaw);
    var masked = configured ? SecretMask.MaskMiddle(apiKeyRaw ?? string.Empty) : string.Empty;
    var available = effectiveEnabled && configured;

    return new WebSearchStatus(
