...
public sealed class VibeLibrarianAgent : VibeAgentBase
{
    public VibeLibrarianAgent()
    {
        SystemPrompt =
            ...
            Inputs:
            - Research question
            - Materials context (sources/facts) when available

            Tasks:
            - Identify key evidence items (sources/facts) relevant to the question.
            - Point out missing evidence (what file/source is needed).
            - If an axiom/definition is missing and blocks derivation, you may propose writing a small file to fill the gap.
            ...
            JSON schema (all fields optional; use empty arrays when none):
            {
              "factsWrite": [
                {
                  "title": "string",
                  "relativePath": "string (optional, .md/.txt, will be sandboxed under sra/{sessionId}/)",
              ...
              "axiomsForDag": [
                {
                  ...
                  "citation": "string (REQUIRED: where in paper/source this comes from)",
