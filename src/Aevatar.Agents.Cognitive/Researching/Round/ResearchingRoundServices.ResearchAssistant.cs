...
using Aevatar.Agents.Cognitive.Researching.Goals;
...
public sealed partial class ResearchingRoundServices
{
        ...
        CancellationToken ct)
    {
        ...
        catch (Exception ex)
        {
            if (ex is OperationCanceledException && ct.IsCancellationRequested)
                throw;
            _host.Logger.LogDebug(ex, "[VibeRoundServices] research_assistant brief failed (best-effort).");
            return null;
        }
    }
        ...
        CancellationToken ct)
    {
        ...
        catch (Exception ex)
        {
            if (ex is OperationCanceledException && ct.IsCancellationRequested)
                throw;
            _host.Logger.LogDebug(ex, "[VibeRoundServices] research_assistant plan failed (best-effort).");
            return new PlanResult(null, null, null);
        }
    }
        ...
        CancellationToken ct)
    {
        ...
        catch (Exception ex)
        {
            if (ex is OperationCanceledException && ct.IsCancellationRequested)
                throw;
            _host.Logger.LogDebug(ex, "[VibeRoundServices] research_assistant summary failed (best-effort).");
            return null;
        }
    }
