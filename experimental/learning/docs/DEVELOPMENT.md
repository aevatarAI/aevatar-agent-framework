...
- 默认：后端 + **Tauri 桌面端**
- Web 模式（仅 Vite）：`./experimental/learning/boot.sh --web`
...
> `experimental/learning/boot.sh` 默认会尝试自动清理端口（macOS/Linux，依赖 `lsof`）。
...
参考：`experimental/novel/frontend/src-tauri/tauri.conf.json` 的 `app.security.csp` 配置方式。
