...
5. **运行 workers（按确定性顺序）**
   ...
   - `librarian` 的输出可能触发副作用：写 DAG facts、提供“可信 axiom 候选”（给 dag_builder 参考）
...
6. **DAG apply（当前实现：no verification / no consensus）**
   ...
   - 若候选 mutation 引用缺失的证据，会在输出中显式标注缺口
...
- `materials.RenderedContext`（DAG facts 的 bounded 摘要）
...
- **主 assistant 合并流**：`msg:{sessionId}:assistant:{runId}`
  - 含章节化输出：Brief（若首次生成）、Plan（RA）、Round Summary、以及一些系统式提示（如 librarian 写 DAG facts、delivery center 更新等）
