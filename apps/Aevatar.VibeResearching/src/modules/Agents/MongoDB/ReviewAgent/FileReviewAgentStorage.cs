...
public sealed class FileReviewAgentStorage : IReviewAgentStorage
{
    ...
    public async Task<object> GetIterationsAsync(int limit, int offset, CancellationToken ct)
    {
        return await LoadIterationsAsync(limit, offset, ct);
    }

    public async Task<object?> GetIterationAsync(string iterationId, CancellationToken ct)
    {
        return await LoadIterationAsync(iterationId, ct);
    }

    public async Task<object> GetCurrentIterationEntriesAsync(CancellationToken ct)
    {
        var state = await LoadStateAsync(ct);
        return state?.CurrentIterationEntries ?? new List<ReviewLogEntry>();
    }
}
