### experimental/novel/src - .NET Sidecar 实现目录
...
- `Aevatar.Novel.Contracts/`
  - Protobuf 契约生成（从 `experimental/novel/protos/*.proto` 生成 C# 类型）
...
在 `artifacts/tests/narrative_tests.md` 中写一个 ` ```json ` 代码块（见 `experimental/novel/agent_skills/narrative-tests/SKILL.md`），即可被 v1 runner 执行。
