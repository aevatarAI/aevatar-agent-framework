...
export default function ComputePanel(props: {
  ...
  const { transport, sessionId, connected, lastDecision, onDecision } = props;
  ...
  async function post(action: "execute" | "degrade" | "skip") {
    ...
    try {
      ...
      if (!postJson) throw new Error("Transport does not support POST JSON.");
      ...
      const json: any = await postJson(`/api/sessions/${encodeURIComponent(sessionId)}/compute/decision`, body);
