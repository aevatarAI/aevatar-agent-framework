...
public sealed class SessionFilesService
{
    ...
    public FileNode ListTree(string sessionId, string? relativeDir = null, int maxDepth = 6)
    {
        ...
        {
            visited++;
            if (visited > MaxListEntries) return new FileNode(relPath, Path.GetFileName(dirPath), "dir", 0, UtcMs(), []);
    ...
    {
        if (string.IsNullOrWhiteSpace(a)) return NormalizeRelative(b);
        return NormalizeRelative($"{a}/{b}");
    }
    ...
    private static bool IsAllowedText(string path)
    {
