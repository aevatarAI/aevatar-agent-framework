...
public sealed class ResearchRuntime
{
    ...
    private readonly ISkillPacksSyncService _skillPacksSync;
    private readonly TimeSpan _skillPacksRetryMinInterval;
    private readonly GlobalAgentYamlRegistry _roles;
    private readonly IVibeSessionRepository? _sessionStore;
    ...
    public ResearchRuntime(
        ...
        ISkillPacksSyncService skillPacksSync,
        IOptions<SkillPacksOptions> skillPacksOptions,
        GlobalAgentYamlRegistry roles,
        IVibeSessionRepository? sessionStore = null)
    {
    ...
    public async Task<AevatarAIAgentState?> TryGetAgentStateAsync(string sessionId, CancellationToken ct)
    {
        ...
        try
        {
            ...
            // Try to get agent state via GetAgent() (only works in Local runtime, not Orleans)
            var agent = entry.MainActor.GetAgent();
            if (agent is AIGAgentBase aiAgent)
            {
                return aiAgent.GetState();
            }
