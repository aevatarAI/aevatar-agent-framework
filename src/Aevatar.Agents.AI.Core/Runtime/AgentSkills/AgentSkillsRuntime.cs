...
internal sealed partial class AgentSkillsRuntime
{
    ...
    internal IReadOnlyList<string> GetEffectiveRoots()
    {
        ...
        lock (_rootsLock)
        {
            ...
            hasExplicitRoots = _roots.Count > 0;
        }

        var env = Environment.GetEnvironmentVariable(AgentSkillsRootsEnv);
        var hasEnvRoots = !string.IsNullOrWhiteSpace(env);
        ...
        //
        // Only use default root when caller did NOT provide any explicit roots and no env override is set.
        // This improves determinism (especially for tests) and matches user intent: explicit roots should fully control discovery scope.
        if (!hasExplicitRoots && !hasEnvRoots)
        {
            try
            {
