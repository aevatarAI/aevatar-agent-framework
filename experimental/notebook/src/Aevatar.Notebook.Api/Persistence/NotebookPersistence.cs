...
namespace Aevatar.Notebook.Persistence;

// ============================================================
//  NotebookPersistence
//
//  Goal:
//  - Config-driven persistence selection for Notebook app:
//    - IMemoryStore:       file | mongodb | supabase
//    - IMemoryVectorIndex: file | mongodb | supabase
//    - IMemoryGraphStore:  file | neo4j
//
//  Notes:
//  - "file" uses core defaults (env-var aware, best-effort).
//  - DB providers register infra here and use GAgentOptions to replace defaults.
// ============================================================
internal static class NotebookPersistence
{
    ...
    public static NotebookPersistenceResult Configure(
        ...
        IConfiguration configuration)
    {
        ...
        var selection = new NotebookPersistenceSelection
        {
        ...
        return new NotebookPersistenceResult(ConfigureStores: go =>
        {
    ...
    private static NotebookPersistenceOptions BindOptions(IConfiguration configuration)
    {
        var opt = new NotebookPersistenceOptions();
        configuration.GetSection(SectionPath).Bind(opt);
        return opt;
    }

    private static void ApplyFileRootEnvironmentOverrides(NotebookFileRootOptions file)
    {
    ...
    private static NormalizedSelection NormalizeSelection(NotebookPersistenceOptions options)
    {
    ...
    private static string NormalizeProviderKey(string? raw)
    {
        raw ??= string.Empty;
        var k = raw.Trim();
        return k.Length == 0 ? "file" : k.ToLowerInvariant();
    }

    private static void RegisterMongoDbInfraIfNeeded(
        IServiceCollection services,
        IConfiguration configuration,
        NotebookPersistenceOptions options,
        NormalizedSelection normalized)
    {
        ...
        if (string.IsNullOrWhiteSpace(conn))
        {
            throw new InvalidOperationException(
                ...
                "Please set it in experimental/notebook/src/Aevatar.Notebook.Api/appsettings.secrets.json (or env var).");
        }
    ...
    internal sealed record NotebookPersistenceResult(
        Action<GAgentOptions> ConfigureStores,
        NotebookPersistenceSelection Selection);

    internal sealed class NotebookPersistenceSelection
    {
    ...
    {
        public bool UsesMongoDb => MemoryStoreProvider == "mongodb" || MemoryVectorIndexProvider == "mongodb";
        public bool UsesSupabase => MemoryStoreProvider == "supabase" || MemoryVectorIndexProvider == "supabase";
        public bool UsesNeo4j => MemoryGraphProvider == "neo4j";
    }

    internal sealed class NotebookPersistenceOptions
    {
        public string MemoryStore { get; set; } = "file";
        public string MemoryVectorIndex { get; set; } = "file";
        public string MemoryGraph { get; set; } = "file";

        public NotebookFileRootOptions File { get; set; } = new();
    ...
    internal sealed class NotebookFileRootOptions
    {
