...
public sealed partial class VibeOrchestrator
{
        ...
        CancellationToken ct)
    {
        try
        {
            ...
            var msg = BuildBriefMessage(question, dag, recentTrace, null, input.AttachmentPaths);
        ...
        CancellationToken ct)
    {
        ...
        {
            var (ra, raId) = await _core.Runtime.GetResearchAssistantAgentAsync(sessionId, providerOverride, ct);
            var msg = BuildPlanMessage(question, dag, recentTrace, null, input.AttachmentPaths);
