...
public sealed class MultiAgentAgUiWorkflowTests
{
    ...
    private static async Task<WorkflowRunData> RunComplexWorkflowAsync()
    {
        ...
        try
        {
            ...
            var coordinatorActor = await actorManager.CreateAndRegisterAsync<WorkflowCoordinatorAgent>("coordinator");
            var coordinator = (WorkflowCoordinatorAgent)coordinatorActor.GetAgent();
