        ...
        <div className="space-y-2 max-h-[55vh] overflow-auto">
          {ordered.map((it, idx) => (
            <div key={`${it.runId || "x"}:${it.roundIndex ?? idx}:${idx}`} className="border border-slate-200 rounded-lg p-2 bg-slate-50">
              ...
              {it.workflow && <div className="mt-1 text-[11px] text-slate-600 font-mono break-words">workflow: {it.workflow}</div>}
                ...
                <div className="mt-2 text-[11px] text-rose-700">
                  ...
                  <ul className="mt-1 list-disc pl-4 space-y-1">
                    {it.redFlags.map((rf, i) => (
                      <li key={`${i}:${rf}`} className="break-words">
                        {rf}
                      </li>
                    ))}
                  </ul>
              ...
              {it.summaryPath && <div className="mt-1 text-[11px] text-slate-600 font-mono break-all">summary: {it.summaryPath}</div>}
