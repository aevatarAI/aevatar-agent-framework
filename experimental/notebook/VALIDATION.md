...
本文件用于验证 `experimental/notebook/Aevatar.Notebook` 在不同持久化后端组合下可用（Sources / Q&A / Report / Trace / Graph）。
...
默认地址见 `experimental/notebook/src/Aevatar.Notebook.Api/appsettings.json`：`http://localhost:5678`
...
在 `experimental/notebook/src/Aevatar.Notebook.Api/appsettings.secrets.json` 填：

- `ConnectionStrings:MongoDB`
- `MongoDB:Database`（默认 `aevatar`）

并在 `experimental/notebook/src/Aevatar.Notebook.Api/appsettings.json` 或 secrets 覆盖：
...
确保 Neo4j 可访问（bolt），并在 `experimental/notebook/src/Aevatar.Notebook.Api/appsettings.secrets.json` 填：
...
- **vector 不工作**：检查 `experimental/notebook/src/Aevatar.Notebook.Api/appsettings.secrets.json` 中 `LLMProviders:Providers:default:Embeddings:Enabled`
- **graph 没投影**：
