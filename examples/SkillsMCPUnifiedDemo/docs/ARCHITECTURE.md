...
目的：提供一个最小可跑的“统一 demo”，把 **Agent Skills / dotnet-file skills / python-file skills / MCP tools** 串起来。
...
- `UnifiedAgent`：继承 `AIGAgentBase`，在 `RegisterToolsAsync` 里统一注册：
  ...
  - file-based tools（自动扫描 `skills/` 目录；需要 manifest 才会被注册）
    - dotnet-file：`dotnet run --file`
    - python-file：`python3 -I -u`（可用环境变量 `AEVATAR_PYTHON_BIN` 覆盖 python 路径）
