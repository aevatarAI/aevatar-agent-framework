...
namespace Aevatar.VibeResearching.Infrastructure.MongoDB.Trace;

// ============================================================
//  TraceRepository (Derivation Trace, File-SSoT)
//
//  Files:
...
public sealed class TraceRepository : ITraceRepository
{
    ...
    public TraceRepository(WorkspaceService workspace, ILogger<TraceRepository> logger)
    {
    ...
    public async Task<object> GetSnapshotForUiAsync(string sessionId, int max, CancellationToken ct)
    {
        var summaries = await LoadLatestAsync(sessionId, max, ct);
        var ws = _workspace.EnsureSessionWorkspace(sessionId);
