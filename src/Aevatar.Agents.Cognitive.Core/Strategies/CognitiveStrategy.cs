...
public sealed class CognitiveStrategy : IReasoningStrategy
{
        ...
        IAIAgentEmbeddingFactory? embeddingFactory = null)
    {
        ...
        {
            searchPaths.Add(Path.Combine(repoRoot, "src", "Aevatar.Agents.Cognitive", "workflows"));
        }
        ...
        CancellationToken ct = default)
    {
        ...
        try
        {
            ...
            coordinator.SetStepEventCallback(stepEvent =>
            {
                ...
                Aevatar.Agents.Cognitive.Core.VotingProgress? votingProgress = null;
                if (stepEvent.StepType == "vote" && stepEvent.VoteK > 0)
                {
                    votingProgress = new Aevatar.Agents.Cognitive.Core.VotingProgress
                    {
                        Type = "Consensus",
