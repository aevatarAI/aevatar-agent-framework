...
public interface IReviewAgentService
{
    ...
    /// <summary>
    /// Get the current state of the Review Agent (async version).
    /// </summary>
    Task<object> GetStatusAsync(CancellationToken ct = default);

    /// <summary>
    /// Get current Review Agent settings (async version).
    /// </summary>
    Task<object> GetSettingsAsync(CancellationToken ct = default);

    /// <summary>
    /// Update Review Agent settings (async version with object parameter).
    /// </summary>
    Task<object> UpdateSettingsAsync(object settings, CancellationToken ct = default);

    /// <summary>
    /// Get graph data for review progress visualization (async version).
    /// </summary>
    Task<object> GetReviewGraphAsync(CancellationToken ct = default);

    /// <summary>
    /// Triggers a review round (async version).
    /// </summary>
