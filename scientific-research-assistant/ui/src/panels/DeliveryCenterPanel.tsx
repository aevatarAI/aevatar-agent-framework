              ...
              {conclusions.length === 0 ? (
                ...
                conclusions.slice(0, 20).map((c: any, idx: number) => (
                  <div key={`${c?.cardId ?? idx}:${idx}`} className="border border-slate-200 rounded-lg p-2 bg-slate-50">
                    ...
                    {c?.notes && <div className="mt-1 text-[11px] text-slate-600 whitespace-pre-wrap break-words">{String(c.notes)}</div>}
                    {Array.isArray(c?.evidencePaths) && c.evidencePaths.length > 0 && (
                      <div className="mt-2 text-[11px] text-slate-600 break-words">
                        evidence: <span className="font-mono text-slate-500">{c.evidencePaths.slice(0, 6).join(", ")}</span>
                      </div>
                    )}
                  </div>
                ))
              ...
              {evidence.length === 0 ? (
                ...
                evidence.slice(0, 30).map((e: any, idx: number) => (
                  <div key={`${e?.evidenceId ?? idx}:${idx}`} className="border border-slate-200 rounded-lg p-2 bg-slate-50">
                    ...
                    {e?.excerpt && <div className="mt-1 text-[11px] text-slate-600 whitespace-pre-wrap break-words">{String(e.excerpt)}</div>}
                    {e?.relevance && (
                      <div className="mt-1 text-[11px] text-slate-500 whitespace-pre-wrap break-words">relevance: {String(e.relevance)}</div>
                    )}
                  </div>
                ))
              )}
                ...
                tasks
                  ...
                  .map((t: any, idx: number) => (
                    <div key={`${t?.taskId ?? idx}:${idx}`} className="border border-slate-200 rounded-lg p-2 bg-slate-50">
                      ...
                      {t?.detail && <div className="mt-1 text-[11px] text-slate-600 whitespace-pre-wrap break-words">{String(t.detail)}</div>}
