...
namespace Aevatar.VibeResearching.Infrastructure.MongoDB.Knowledge.Dag;
...
public sealed class DagStore : Aevatar.VibeResearching.Knowledge.IDagStore
{
    private const int MaxNodesForList = 2000;
    private const int MaxEdgesForList = 8000;
    private const int MaxStagedList = 80;
    private const string GlobalDagId = "global"; // from SessionConsts
    ...
    public async Task<object> GetSnapshotForListAsync(string dagId, CancellationToken ct, string? currentSessionId = null)
    {
        ...
        if (dagId == GlobalDagId && !string.IsNullOrWhiteSpace(currentSessionId))
        {
            try
            {
