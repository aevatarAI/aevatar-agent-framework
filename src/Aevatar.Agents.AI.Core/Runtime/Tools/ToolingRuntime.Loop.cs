...
internal sealed partial class ToolingRuntime
{
    ...
    private const int MaxSameToolCallAttempts = 2;
        ...
        CancellationToken cancellationToken)
    {
        ...
        for (var round = 0; round < MaxToolCallRounds && current.AevatarFunctionCall != null; round++)
        {
            ...
            var attempts = IncrementToolCallAttempts(toolCallAttempts, signature);

            // If the model keeps requesting the exact same tool call, force it to answer without tools.
            if (attempts > MaxSameToolCallAttempts)
            {
                _loopHost.Logger.LogWarning(
                    ...
                    attempts, MaxSameToolCallAttempts, functionCall.Name, request.RequestId);
            ...
            var reasoningContent = round == 0 ? TryGetReasoningContent(initialResponse) : null;
            ...
            AppendToolMessages(llmRequest, functionCall, callId, toolResult, reasoningContent);
            PersistToolTranscriptToHistoryBestEffort(llmRequest);
            await PublishToolExecutionResponseEventAsync(request, functionCall.Name, toolResult, cancellationToken);
        ...
        if (current.AevatarFunctionCall != null)
        {
            _loopHost.Logger.LogWarning(
                ...
                MaxToolCallRounds, request.RequestId);

            current = await ForceModelToAnswerWithoutToolsAsync(
                llmRequest,
                reason: $"Exceeded max tool call rounds ({MaxToolCallRounds}).",
                cancellationToken);
    ...
    private static string? TryGetReasoningContent(AevatarLLMResponse response)
    {
        if (response.Metadata == null)
            return null;
        if (!response.Metadata.TryGetValue("reasoning_content", out var obj))
            return null;
        return obj as string;
    }

    private static int IncrementToolCallAttempts(Dictionary<string, int> toolCallAttempts, string signature)
    {
    ...
    private void PersistToolTranscriptToHistoryBestEffort(AevatarLLMRequest llmRequest)
    {
        if (!_loopHost.EnableChatHistoryInState)
            return;

        if (llmRequest.Messages.Count < 2)
            return;

        var toolCallMsg = llmRequest.Messages[^2];
        var toolResultMsg = llmRequest.Messages[^1];

        _loopHost.AddMessageToHistory(toolCallMsg);
        _loopHost.AddMessageToHistory(toolResultMsg);
    }

    private async Task PublishToolExecutionResponseEventAsync(
        ChatRequest request,
        string toolName,
        ToolExecutionResult toolResult,
