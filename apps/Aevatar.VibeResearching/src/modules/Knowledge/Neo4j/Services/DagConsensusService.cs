...
using DomainConsensusInput = Aevatar.VibeResearching.Knowledge.ConsensusInput;
using DomainConsensusResult = Aevatar.VibeResearching.Knowledge.ConsensusResult;

namespace Aevatar.VibeResearching.Knowledge.Neo4j.Services;

// ============================================================
//  DagConsensusService (consensus gate)
//
//  Goal:
...
public sealed partial class DagConsensusService : IDagConsensusService
{
    ...
    private static readonly Google.Protobuf.JsonParser ProtoParser =
        new(Google.Protobuf.JsonParser.Settings.Default.WithIgnoreUnknownFields(true));
    ...
    private readonly ILogger<DagConsensusService> _logger;

    public DagConsensusService(
        IConfiguration configuration,
        ResearchRuntime runtime,
        CognitiveStrategy cognitive,
        IWorkspaceService workspace,
        IDagRepository dagRepository,
        ILogger<DagConsensusService> logger)
    {
    ...
    /// <summary>
    /// Explicit interface implementation bridging domain-level simple types to the rich internal API.
    /// </summary>
    async Task<DomainConsensusResult> IDagConsensusService.RunAsync(DomainConsensusInput input, CancellationToken ct)
    {
        ArgumentNullException.ThrowIfNull(input);
        ct.ThrowIfCancellationRequested();

        SraDagMutation candidate;
        if (string.IsNullOrWhiteSpace(input.MutationCandidate))
        {
            candidate = new SraDagMutation { SessionId = input.SessionId };
        }
        else
        {
            try
            {
                candidate = ProtoParser.Parse<SraDagMutation>(input.MutationCandidate);
            }
            catch (Exception ex)
            {
                return new DomainConsensusResult
                {
                    IsValid = false,
                    Reasoning = "json_parse_failed",
                    Issues = [$"Failed to parse MutationCandidate: {ex.Message}"]
                };
            }
        }

        var snapshot = await _dagRepository.LoadSnapshotAsync(input.SessionId, ct);

        var richInput = new ConsensusInput(
            SessionId: input.SessionId,
            RunId: Guid.NewGuid().ToString("N"),
            Current: snapshot,
            Candidate: candidate);

        var result = await RunAsync(richInput, ct);

        return new DomainConsensusResult
        {
            IsValid = result.Ok,
            Reasoning = result.Error ?? result.Workflow,
            Issues = result.RedFlags?.ToList() ?? []
        };
    }
