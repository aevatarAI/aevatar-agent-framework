...
- **推荐（一次配置，全仓复用）**：把真实密钥写入用户级 secrets（加密）：
  ...
  - 写入方式：
    ...
    - Secrets App：`apps/Aevatar.Config`（Web）
    - CLI：`src/Aevatar.Agents.SecretsCli`（见 `src/Aevatar.Agents.Core/docs/UserSecrets.md`）
- **可选（项目级覆盖）**：`src/VibeResearching.Api/appsettings.secrets.json`（本地文件、不要提交）
...
位置：
- 推荐：`src/VibeResearching.Api/appsettings.json` 的 `MCP:mcpServers`
- 也支持：直接提供 Cursor 原生 `mcp.json`（root 级 `mcpServers`）
  - 本项目已在 `Program.cs` 中 `AddJsonFile("mcp.json", optional: true)`，放在 `src/VibeResearching.Api/mcp.json` 即可（不提交，参考 `mcp.json.example`）
...
- Aevatar 这边不需要“集成插件本体”，只要把 **同一份 Cursor/OpenCode 风格的 `mcpServers` 配置**（例如你本机的 `~/.cursor/mcp.json`）拷贝/同步到 `src/VibeResearching.Api/mcp.json` 即可自动连接并注册工具。
- 仓库内的 `src/VibeResearching.Api/mcp.json.example` 已预留了 `context7`（HTTP/SSE）与 `grep_app`（stdio/npx）的字段模板；把 `enabled` 打开并填好 `url/authToken/command/args` 就能跑。
...
位置：`src/VibeResearching.Api/skillpacks.json`（不提交，参考 `skillpacks.json.example`）
...
- `Materials:MaxFiles`：最多纳入的 DAG 节点数量（有界）
- `Materials:MaxFileChars`：每个节点内容的最大字符数
- `Materials:MaxContextChars`：注入 LLM 的总字符上限（避免上下文爆炸）
- `Materials:MaxPerDocChars`：单条事实在 prompt 中的最大字符数
...
- 当前写入用途：
  ...
  - promote 时写入 DAG（knowledge node）
...
- `src/VibeResearching.Api/appsettings.json`：`Python:Enabled=true`
- 可选覆盖 python 可执行文件：
...
- **后端端口**：默认 `5678`（仓库政策：禁止 `5000`）
  - `BACKEND_PORT=5679 ./boot.sh` 覆盖
  - 或 `ASPNETCORE_URLS=http://localhost:5678`
- **前端端口**：默认 `5173`
  - `FRONTEND_PORT=5174 ./boot.sh` 覆盖
- **前端代理目标**：`VITE_API_BASE_URL`（优先）或 `BACKEND_PORT`
  - `VITE_API_BASE_URL=http://localhost:5678 npm run dev`
