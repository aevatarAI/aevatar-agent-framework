...
public class RiskManagerAgent : AIGAgentBase
{
    ...
    private IExchangeAccountClient? _accountClient;
    private ExchangeCapabilities _capabilities = ExchangeCapabilities.Minimal;

    public IExchangeClient ExchangeClient
    {
        set
        {
            _accountClient = value.Account;
    ...
    /// <summary>
    /// Handle startup risk check request
    /// </summary>
    [EventHandler]
    public async Task HandleStartupRiskCheck(StartupRiskCheckRequestedEvent evt)
    {
        var positions = await LoadPositionsSafeAsync();
        if (positions.Count == 0)
        {
            await PublishStartupRiskResult(evt.RequestId, false, StartupRiskAction.Hold, "no positions", "no positions detected");
            return;
        }
        ...
        if (equity <= 0)
        {
            await PublishStartupRiskResult(evt.RequestId, true, StartupRiskAction.Reduce, "equity unknown", "positions exist but equity is unknown");
            return;
        }

        var ratio = totals.TotalNotional / equity * 100;
        if (totals.UnrealizedPct <= -_maxDailyLossPct)
        {
            await PublishStartupRiskResult(evt.RequestId, true, StartupRiskAction.CloseAll, "loss exceeds daily limit", "startup risk: unrealized loss exceeds limit");
            return;
        }
        ...
        await PublishStartupRiskResult(evt.RequestId, false, StartupRiskAction.Hold, "within limits", "startup risk check passed");
    }

    /// <summary>
    /// Apply policy updates (risk thresholds)
    /// </summary>
    [EventHandler]
    public Task HandleTradingPolicyUpdated(TradingPolicyUpdatedEvent evt)
    {
        if (evt.Policy != null)
        {
            ...
            Logger.LogInformation("[RiskManager] Policy updated by {By}", evt.UpdatedBy);
        }
    ...
    private void ApplyPolicy(TradingPolicyConfig policy)
    {
        ...
        {
            _maxPositionPct = policy.Trading.MaxPositionPct;
            _maxTotalPositionPct = policy.Trading.MaxTotalPositionPct;
            _maxLossPerTrade = policy.Trading.MaxLossPerTrade;
            _maxDailyLossPct = policy.Trading.MaxDailyLoss;
            _minConfidenceToTrade = policy.Trading.MinConfidenceToTrade;
        }

        if (policy.Risk != null)
        {
            _maxConsecutiveLosses = policy.Risk.MaxConsecutiveLosses;
            _cooldownMinutes = policy.Risk.CooldownMinutes;
        }
    }

    private async Task<IReadOnlyList<PositionInfo>> LoadPositionsSafeAsync()
    {
        if (_accountClient == null || !_capabilities.SupportsPositions)
            return Array.Empty<PositionInfo>();

        try
        {
            return await _accountClient.GetPositionsAsync();
        }
        catch (Exception ex)
        {
            Logger.LogWarning(ex, "[RiskManager] Load positions failed (non-fatal)");
            return Array.Empty<PositionInfo>();
        }
    }

    private (double TotalNotional, double UnrealizedPct) CalculatePositionTotals(IReadOnlyList<PositionInfo> positions)
    {
        ...
        foreach (var position in positions)
        {
            var notional = position.Notional
                           ?? (position.MarkPrice.HasValue ? position.MarkPrice.Value * position.Size : 0m);
            totalNotional += Math.Abs((double)notional);

            if (position.UnrealizedPnl.HasValue)
                totalUnrealized += (double)position.UnrealizedPnl.Value;
        }
    ...
    private async Task PublishStartupRiskResult(
        string requestId,
        bool actionRequired,
        StartupRiskAction action,
        string reason,
        string summary)
    {
        await PublishAsync(new StartupRiskCheckResultEvent
        {
            RequestId = requestId,
            ...
            Summary = summary ?? "",
            Timestamp = Timestamp.FromDateTime(DateTime.UtcNow)
        }, Aevatar.Agents.EventDirection.Down);
    }
