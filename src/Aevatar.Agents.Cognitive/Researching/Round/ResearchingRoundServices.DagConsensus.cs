...
public sealed partial class ResearchingRoundServices
{
    ...
    private async Task<DagRoundResult> RunDagApplyAsync(
        ...
        Action<string> emit,
        string? providerName,
        IWorkflowRunEventSink eventSink,
        CancellationToken ct)
    {
        ...
        try
        {
            var progress = BuildDagConsensusAgUiProgress(eventSink, runId, workflowName: "maker");
    ...
    private static IProgress<ReasoningProgress> BuildDagConsensusAgUiProgress(
        ...
        string workflowName)
    {
        var emitter = new AgUiExecutionTraceEmitter(eventSink);
        ...
        return new Progress<ReasoningProgress>(progress =>
        {
            try
            {
                ...
                emitter.Emit(evt, status, stepName);
            }
            catch
            {
                // best-effort
