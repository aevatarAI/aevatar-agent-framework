...
// ============================================================
//  SessionUiSnapshotStore (File-backed UI work trace)
//
//  WHY:
...
//
//  NOTE:
//  - This is best-effort telemetry; failures must not break runs.
...
public sealed class SessionUiSnapshotStore
{
    ...
    public async Task SaveAsync(string sessionId, UiSnapshot snapshot, CancellationToken ct)
    {
        ...
        {
            _logger.LogDebug(ex, "Failed to write ui_snapshot.json (best-effort).");
        }
    }
