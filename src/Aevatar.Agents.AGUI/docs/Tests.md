...
| 运行时/生命周期 | 不适用 | N/A | — |
| 事件/订阅/流 | 不适用 | N/A | — |
| 序列化/Protobuf | 适用 | ✅ 已覆盖（至少有相关测试） | `test/Aevatar.Agents.AGUI.Tests/AgUiBootstrapTests.cs` |
| 持久化/存储 | 不适用 | N/A | — |
| AI/LLM | 不适用 | N/A | — |
| 工具调用 | 不适用 | N/A | — |
| Hooks/观测 | 不适用 | N/A | — |
| CQRS/EventSourcing | 不适用 | N/A | — |
| AG-UI/SSE | 适用 | ✅ 已覆盖（至少有相关测试） | `test/Aevatar.Agents.AGUI.Tests/AgUiBootstrapTests.cs` |
