...
- 把每一步的产物沉淀成可复用知识：先进入 `facts_proposed/`，经 multi-agent 共识/验证后再 promote 到 DAG
...
1. **资料层：DAG / facts_proposed（文件即真相）**
   - `facts_proposed/`：候选事实（未通过共识/验证前不可当作前提依赖）
   - DAG knowledge nodes：已通过共识/验证的事实（可当作事实依赖）
...
2. **Multi-agent 推理、验证与共识**
   ...
   - 通过后由系统 promote：`facts_proposed/` → DAG
...
5. **可视化与实时性**
   ...
   - Workspace state（DAG facts/facts_proposed 计数与预览）随会话更新
...
- **Fact yield**：每次 session 产出可写回 DAG 的条目数量
...
2. **Grounded**：任何关键结论必须指向 DAG facts（或明确标注来自 `facts_proposed/`）
...
- **Key Metrics Displayed**: run/step 状态、tool 调用、workspace 变更（DAG facts/facts_proposed）

## Future Vision

### Potential Enhancements

- **Notebook-style indexing**：DAG facts（可选含 facts_proposed）→ MemoryStore + VectorIndex（高质量检索）
