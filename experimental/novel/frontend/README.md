...
哥，这个目录是 `experimental/novel/` 的桌面前端：**Tauri + React + Monaco**，目标是提供接近 Cursor/VSCode 的写作编辑体验，并通过 **Protobuf 契约**与 `.NET sidecar` 通信（SSE + Protobuf JSON）。
...
本项目 UI↔Sidecar 的跨边界消息必须 Protobuf（见 `experimental/novel/protos/*.proto`）。
...
```bash
cd experimental/novel/src/Aevatar.Novel.Sidecar
dotnet run
```
...
```bash
cd experimental/novel/frontend
npm install
npm run dev
```
...
Sidecar 读取 `NovelAI` 配置（`experimental/novel/src/Aevatar.Novel.Sidecar/appsettings.json`），也支持用环境变量覆盖：
...
- **跨边界消息只认 Protobuf**：HTTP 传 Protobuf JSON，SSE 推 Protobuf JSON（见 `experimental/novel/protos/novel_sidecar.proto` + `experimental/novel/src/Aevatar.Novel.Sidecar/Program.cs`）。
