        ...
        try
        {
            ...
            if (string.IsNullOrWhiteSpace(providerOverride))
            {
                try
                {
                    ...
                    if (snap != null && snap.Map.TryGetValue("research_assistant", out var p) && !string.IsNullOrWhiteSpace(p))
...
// ============================================================
// ResearchStreamEventContext and IResearchStreamEventSink
// These types were in the old VibeResearching.Streaming namespace
// but don't exist in the new structure yet.
// ============================================================

/// <summary>
/// Context for streaming research events to UI.
/// Uses AsyncLocal for thread-safe context propagation.
/// </summary>
public static class ResearchStreamEventContext
{
    private static readonly AsyncLocal<IResearchStreamEventSink?> _current = new();
...
/// <summary>
/// Interface for sinks that receive research tool progress events.
/// </summary>
public interface IResearchStreamEventSink
{
    Task EmitToolStartAsync(string toolCallId, string toolName, CancellationToken ct);
    Task EmitToolProgressAsync(string toolCallId, string toolName, string message, CancellationToken ct);
    Task EmitToolEndAsync(string toolCallId, string toolName, bool success, long durationMs, string? error, string? resultPreview, CancellationToken ct);
}
