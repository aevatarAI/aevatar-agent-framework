...
/// <summary>
/// Session UI trace recorder implementation.
/// Attaches to ResearchSession's AG-UI event hub and persists UI snapshots and per-run JSONL work traces.
/// All persistence is best-effort and never blocks runs.
/// </summary>
public sealed class SessionUiTraceRecorder : ISessionUiTraceService
{
    ...
    public void Attach(object session)
    {
        if (session is not ResearchSession researchSession)
        {
            _logger.LogWarning("Attach called with non-ResearchSession object, ignoring.");
            return;
        }

        ArgumentNullException.ThrowIfNull(researchSession);

        if (!_attached.TryAdd(researchSession.Id, 1))
            return;

        _ = Task.Run(() => RunAsync(researchSession));
    }
