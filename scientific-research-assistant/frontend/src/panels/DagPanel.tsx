...
export default function DagPanel(props: {
  ...
  const { sessionId, connected, snapshot, onSnapshot } = props;
  ...
  const filtered = useMemo(() => {
    ...
    const list = qq
      ? nodes.filter((n) => (n.id || "").toLowerCase().includes(qq) || (n.label || "").toLowerCase().includes(qq))
      : nodes;
  ...
  async function refresh() {
    ...
    setError("");
    try {
      const res = await fetch(`/api/sessions/${encodeURIComponent(sessionId)}/dag`);
  ...
  return (
    <div className="space-y-2">
      <div className="flex items-center gap-2">
        ...
        <button
          onClick={() => window.open(`/?view=dag&session=${encodeURIComponent(sessionId)}`, "_blank")}
          ...
          title="Open DAG graph in a new tab"
        >
          Open
        </button>
      ...
      <DagGraphView
        ...
        heightPx={240}
        selectedId={selected}
        onSelect={(id) => void loadExplain(id)}
      />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        ...
        <div className="border border-slate-200 rounded-lg bg-slate-50 p-2">
          ...
          {explain ? (
            <pre className="text-xs text-slate-900 whitespace-pre-wrap break-words max-h-72 overflow-auto">
