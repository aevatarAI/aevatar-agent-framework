...
public sealed class UploadsStore
{
        ...
        CancellationToken ct)
    {
        ...
        if (string.IsNullOrWhiteSpace(ext) || !AllowedExtensions.Contains(ext))
            throw new ArgumentException($"unsupported file extension: '{ext}'", nameof(fileName));

        if (length <= 0)
            throw new ArgumentException("empty file", nameof(content));

        if (length > MaxFileBytes)
            throw new ArgumentException($"file too large (max {MaxFileBytes} bytes)", nameof(content));
        ...
        try
        {
            ...
            shortHash = Convert.ToHexString(hash).ToLowerInvariant()[..8];
        }
        finally
        {
            if (content.CanSeek)
