...
在 `apps/Aevatar.VibeResearching/` 下：
...
VITE_API_BASE_URL=http://localhost:5678 npm run dev -- --port 5173
```
...
- 确认后端 `health` OK
- 确认前端代理环境变量：`VITE_API_BASE_URL=http://localhost:5678`

#### 2) MCP tools 注册失败

- 检查 `src/VibeResearching.Api/appsettings.json` 的 `MCP:mcpServers` 是否配置正确
...
#### 4) vibe 模式读不到 DAG facts

- 确认 DAG 有知识节点（UI DAG / `/api/sessions/{id}/dag`）
- 检查 `src/VibeResearching.Api/appsettings.json` 的 `Materials:*` 配置
- UI 里打开 `Workspace`（STATE_SNAPSHOT）确认 materials 是否被加载

#### 5) Paper Collaboration / facts_proposed 不工作

- 默认写入是关闭的：确认 `src/VibeResearching.Api/appsettings.json` 的 `Materials:AllowWrite=true`
- 通过 `GET /api/sessions/{id}/workspace` 看文件快照（facts_proposed 计数与最近提案）
...
```bash
dotnet test apps/Aevatar.VibeResearching/test/VibeResearching.Api.Tests/VibeResearching.Api.Tests.csproj
```
