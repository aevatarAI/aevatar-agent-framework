...
public class DataCollectorAgent : GAgentBase<DataCollectorState>
{
    ...
    private IExchangeMarketDataClient? _marketDataClient;
    private IExchangeMarketStreamClient? _marketStream;
    ...
    public IExchangeClient ExchangeClient
    {
        set
        {
            _marketDataClient = value.MarketData;
            _marketStream = value.MarketStream;
    ...
    protected override async Task OnDeactivateAsync(CancellationToken ct = default)
    {
        ...
        if (_marketStream != null)
        {
            await _marketStream.DisconnectAsync();
        }
    ...
    {
        // NOTE:
        // - "Connected" historically meant WS connected.
        ...
        CancellationToken ct = default)
    {
        ...
        {
            Logger.LogWarning("[DataCollector] MarketData client not configured, skip start.");
            return;
        }
        ...
        if (canStream)
        {
            try
            {
                await _marketStream!.ConnectAsync(ct);
        ...
        foreach (var symbol in symbols)
        {
            ...
            {
                await _marketStream.SubscribeTickerAsync(symbol, ct);
                await _marketStream.SubscribeKlineAsync(symbol, _pollingInterval, ct);
            }
    ...
    {
        if (_marketDataClient == null)
            throw new InvalidOperationException("Market data client not configured");

        var klines = await _marketDataClient.GetKlinesAsync(symbol, interval, limit, ct);
    ...
    // ============ Market Stream Handlers ============

    private void SetupMarketStreamHandlers()
    {
        if (_marketStream == null) return;

        _marketStream.OnTicker += OnTickerReceived;
        _marketStream.OnKline += OnKlineReceived;
        _marketStream.OnConnected += OnStreamConnected;
        _marketStream.OnDisconnected += OnStreamDisconnected;
        _marketStream.OnError += OnStreamError;
    }
    ...
    private void StartPolling(IEnumerable<string> symbols)
    {
        if (_marketDataClient == null)
        {
            Logger.LogWarning("[DataCollector] REST polling fallback skipped: market data client not configured.");
            return;
        }
    ...
    {
        if (_marketDataClient == null) return;
        ...
        try
        {
            foreach (var symbol in symbols)
            {
                var ticker = await _marketDataClient.GetTickerAsync(symbol);
            ...
            if (DateTime.UtcNow >= _nextKlinePollUtc)
            {
                ...
                foreach (var symbol in symbols)
                {
                    var klines = await _marketDataClient.GetKlinesAsync(symbol, _pollingInterval, limit: 1);
    ...
    private void OnTickerReceived(TickerResponse ticker)
    {
        ...
        // Fire and forget - do not block stream reception
    ...
    private void OnKlineReceived(KlineData kline)
    {
        var evt = new KlineUpdateEvent
        {
            ...
            // - For multi-symbol WS support, update stream client to emit (symbol, interval, kline).
    ...
    {
        State.IsConnected = true;
        Logger.LogInformation("[DataCollector] Market stream connected");
    }
    ...
    {
        Logger.LogError("[DataCollector] Market stream error: {Error}", error);
    }
    ...
    private async Task TryReconnectAsync()
    {
        ...
        try
        {
            await _marketStream.ConnectAsync();

            // Resubscribe
            foreach (var symbol in _subscribedSymbols)
            {
                await _marketStream.SubscribeTickerAsync(symbol);
            }
