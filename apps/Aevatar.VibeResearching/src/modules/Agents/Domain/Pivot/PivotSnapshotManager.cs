...
public sealed class PivotSnapshotManager : IPivotSnapshotManager
{
    ...
    public async Task<PivotSnapshotMetadata> CreateSnapshotAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        // Generate a unique pivot ID
        var pivotId = Guid.NewGuid().ToString("N");
        return await CreateSnapshotAsync(sessionId, pivotId, directionSummary, cancellationToken);
    }
    ...
    /// <inheritdoc />
    public Task<object> ListSnapshotsAsync(string sessionId, CancellationToken cancellationToken = default)
    {
        if (!_snapshots.TryGetValue(sessionId, out var sessionSnapshots))
        {
        ...
        var snapshots = sessionSnapshots.Values
            .OrderByDescending(s => s.CreatedAt)
            .Select(s => new
            {
                pivotId = s.PivotId,
                snapshotId = s.SnapshotId,
                directionSummary = s.DirectionSummary,
                createdAt = s.CreatedAt,
                expiresAt = s.ExpiresAt,
                isValid = s.IsValid,
                nodeCount = s.Snapshot.AllNodes.Count()
            })
            .ToList();
...
public interface IPivotSnapshotManager
{
    ...
    /// <summary>
    /// Creates a snapshot with auto-generated pivot ID.
    /// </summary>
    Task<PivotSnapshotMetadata> CreateSnapshotAsync(
        string sessionId,
        string directionSummary,
    ...
    /// </summary>
    Task<object> ListSnapshotsAsync(string sessionId, CancellationToken cancellationToken = default);
}
