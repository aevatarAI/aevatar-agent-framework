...
- **Primary storage (MVP)**: 文件系统 + KnowledgeGraph
  - `workspace/dags/{dagId}/artifacts/dag/*`：DAG 快照镜像（审阅/恢复）
...
1. **DAG nodes 作为事实**
   - *Why*: 统一事实来源，避免多套真相层漂移
   - *Update*: 核心闭环是 `facts_proposed/` → promote → DAG knowledge node
2. **文件目录作为可审计产物**
   - *Why*: 研究过程需要复现与 review；文件系统天然支持版本化与协作
3. **Protobuf schema-first**
   - *Why*: 跨 runtime/跨 agent 的边界必须可演进、可验证、可序列化
4. **facts_proposed → consensus/verify → DAG**
   - *Why*: 把“未验证推论”与“可依赖事实”隔离，避免污染推理前提
5. **Single-writer for paper/DAG writes**
   - *Why*: 多 agent 并发写同一事实会冲突；通过 proposal + 单写者合并保证一致性
