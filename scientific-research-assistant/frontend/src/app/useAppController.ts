...
import { MessageStore, type ChatRole, type Message, type ToolOutput } from "./messageStore";
import { runSkillsSync } from "./skillsSync";
import { applyUiBootstrapCustomEvent } from "./uiBootstrap";
import { agentDisplayName, parseAgUiMessageId } from "./messageId";
...
export function useAppController() {
  ...
  const refreshDeliverables = async () => {
    ...
    if (!sid) return;
    try {
      const res = await fetch(`/api/sessions/${encodeURIComponent(sid)}/deliverables`);
      if (!res.ok) return;
      const json = await res.json().catch(() => null);
      if (json?.brief) setVibeBrief({ sessionId: sid, ...(json.brief as any) });
      if (json?.delivery) setVibeDelivery(json.delivery);
    } catch {
      // best-effort
    }
  };
  ...
  function connectToSession(id: string) {
    ...
    const upsertStepSystem = (stepName: string, status: "running" | "done") => {
      const rid = runIdRef.current || "run";
      const mid = `sys:${sid}:step:${rid}:${stepName}`;
      const text = status === "running" ? `⏳ ${stepName}` : `✅ ${stepName}`;
