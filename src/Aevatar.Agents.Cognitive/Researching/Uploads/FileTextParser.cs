...
public sealed class FileTextParser
{
    ...
    /// <summary>
    /// Extracts text content from the uploaded stream.
    /// </summary>
        ...
        CancellationToken ct = default)
    {
        ...
        try
        {
            ...
            {
                extracted = await ExtractFromTextStreamAsync(content, maxChars, ct);
            }
            else if (ext == ".pdf")
            {
                extracted = await ExtractFromPdfAsync(content, maxChars, ct);
            }
            else
            {
    ...
    private static async Task<string> ExtractFromTextStreamAsync(
        Stream content,
        int maxChars,
        ...
        CancellationToken ct)
    {
        ...
        {
            _logger.LogWarning("PDF text extraction returned empty content for {FileName}", "(stream)");
