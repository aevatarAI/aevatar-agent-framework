...
#
#  USAGE:
#    ./learning/start.sh            # backend + tauri dev (default)
#    ./learning/start.sh --web      # backend + vite web only
#    ./learning/start.sh --no-kill  # do not kill ports before start
# ============================================================
...
usage() {
  cat <<'EOF'
Usage: ./learning/start.sh [--web|--tauri] [--no-kill]
...
Examples:
  ...
  LEARNING_API_PORT=5679 ./learning/start.sh --web
EOF
}
...
if [[ ! -d "$API_DIR" ]]; then
  ...
  echo "Hint: implement tasks under learning/tasks.md first (API scaffold), then re-run." >&2
  exit 1
fi
if [[ ! -d "$FRONTEND_DIR" ]]; then
  echo "Frontend directory not found: $FRONTEND_DIR" >&2
  echo "Hint: implement tasks under learning/tasks.md first (frontend scaffold), then re-run." >&2
  exit 1
fi
...
kill_port() {
  ...
  if [[ -n "$pids" ]]; then
    ...
    echo "  LEARNING_API_PORT=5679 ./learning/start.sh --web" >&2
    return 1
  fi
}
